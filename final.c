...
void fill_screen(uint8_t* p_fb, uint32_t screen_size, uint8_t r, uint8_t g, uint8_t b)
{
	...
	{
		*((uint32_t*)(p_fb + i)) = 24/*lab5.2_fb_var_screeninfo*/;//value of a pixel
	}
}
	...
	while(true){
		...
		for(i = 0; i < HEIGHT; i++)
		{
			for(j = 0; j < WIDTH; j++)
			{
				...
				*((uint32_t*)(p_fb + offset)) = ((uint32_t)r<<16)+((uint32_t)g<<8)+(uint32_t)b /*lab5.2_fb_var_screeninfo*/;//value of a pixel
			}
		}
		sleep(4);
	}
